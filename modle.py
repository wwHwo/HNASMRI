...
OPS = {
    'Global': lambda C, affine: GlobalBlock(32, 16),
...
class GlobalBlock(nn.Module):
    def __init__(self, dim, heads, depth=1, channel=2, kernel_size=3, patch_size=16, dropout=0.):
        super().__init__()
        ...
        self.transformer = Transformer(dim, heads, depth, 4, mlp_dim, dropout)
...
class LocalBlock(nn.Module):
    ...
    def __init__(self, channel=2):
        ...
        self.node3_1 = nn.Sequential(
            nn.ReLU(inplace=False),
            nn.Conv2d(channel, channel, kernel_size=3, stride=1, padding=1, dilation=2, bias=False),
            nn.Conv2d(channel, channel, kernel_size=1, padding=0, bias=False),
            nn.BatchNorm2d(1 * channel, affine=True),
            nn.ReLU(inplace=False), )
        self.node3_2 = nn.Sequential(
            nn.ReLU(inplace=False),
            nn.Conv2d(channel, channel, (9, 1), stride=1, padding=1, bias=False),
            nn.Conv2d(channel, channel, (1, 9), padding=0, bias=False),
            nn.BatchNorm2d(1 * channel, affine=True),
            nn.ReLU(inplace=False), )
    ...
    def forward(self, x):
        ...
        node2_2 = self.node2(x)
        node2 = torch.add(node2_1, node2_2)
        node3_1 = self.node3_1(node1)
        node3_2 = self.node3_2(node2)
